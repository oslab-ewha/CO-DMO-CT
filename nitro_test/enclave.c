...
s_read_credentials(struct aws_allocator *allocator, struct json_object *object)
{
...
struct aws_string *
s_read_region(struct app_ctx *ctx, struct json_object *object)
{
...
ssize_t
s_write_all(int peer_fd, const char *msg, size_t msg_len)
{
	size_t total_sent = 0;
...
int
s_send_status(int peer_fd, int status, const char *msg)
{
	int rc = 0;
...
static void
handle_connection(struct app_ctx *app_ctx, int peer_fd)
{
	char buf[BUF_SIZE] = {0};
	size_t buf_idx = 0;
	ssize_t rc = 0;
...
main(int argc, char **argv)
{
	...
	keypair = aws_attestation_rsa_keypair_new(app_ctx.allocator, AWS_RSA_2048);
	if (keypair == NULL) {
		perror("Could not create keypair");
		exit(1);
	}

	rc = aws_attestation_request(app_ctx.allocator, keypair, &buf);
